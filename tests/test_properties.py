...
ts = st.datetimes(min_value=__import__("datetime").datetime(2000, 1, 1), max_value=__import__("datetime").datetime(2040, 1, 1)).map(lambda d: d.replace(microsecond=0).isoformat() + "Z")
draft = st.fixed_dictionaries({"at": ts, "source": st.sampled_from(["manual", "sim-a", "sim-b"]), "kind": st.sampled_from(["note", "event", "location"]), "tier": st.sampled_from([1, 2, 3]), "payload": payload})
...
def test_any_single_mutation_is_detected(tmp_path_factory, drafts, data):
    ...
    row = rows[i]
    if field == "text": row["payload"]["text"] += "x"
    elif field == "tier": row["tier"] = 1 if row["tier"] != 1 else 2
    elif field == "seq": row["seq"] += 1
    elif field == "prev": row["prev"] = "f" * 64
    else: row[field] = str(row[field]) + "x"
