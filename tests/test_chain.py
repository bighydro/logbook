...
def test_append_keeps_chain_valid(tmp_path):
    ...
    lb.append(at="2026-03-02T09:00:00Z", source="manual", kind="note", tier=2,
...
def test_tampering_is_detected(tmp_path):
    ...
    row = json.loads(lines[3]); row["payload"]["text"] = "edited"; lines[3] = json.dumps(row, sort_keys=True)
    f.write_text("\n".join(lines) + "\n")
...
def test_missing_line_is_detected(tmp_path):
    shutil.copytree(SAMPLE, tmp_path / "lb")
    f = next((tmp_path / "lb" / "logbook").glob("*/*.jsonl"))
    lines = f.read_text().splitlines(); del lines[5]
    f.write_text("\n".join(lines) + "\n")
    _, _, errors = Logbook(tmp_path / "lb").verify()
...
def test_cli_round_trip(tmp_path):
    env = {"LOGBOOK_HOME": str(tmp_path / "lb"), "PATH": "/usr/bin:/bin", "PYTHONPATH": str(ROOT)}
    run = lambda *a: subprocess.run([sys.executable, "-m", "logbook.cli", *a], env=env, capture_output=True, text=True, check=True).stdout
